 ...
 */
template <typename T>
std::vector<std::complex<T>> fft_1d(const std::vector<std::complex<T>> &xi,
                                    const fft_dir &dir) {
  int cnt = (int)xi.size();
  int msb = findMSB(cnt);
...
float rng() { return (double)rand() / (double)RAND_MAX; }
...
*/
...
#if 1 // FFT 3D
  xi = {0, 1, 2, 3, 4, 5, 6, 7};
  xi = {-0.0885617,  0.955652,   0.886429,   0.924431,  -0.395304,  -0.0665829,
        ...
        -0.74844,    -0.45548,   0.211495,   0.343527,  -0.515198,  -0.0163946,
        0.125886,    0.0983918,  -0.036781,  0.288511,  -0.164898,  0.409043,
        0.922663,    0.807615,   0.740989,   0.263322,  -0.215215,  -0.127425,
        0.136959,    -0.0341827, -0.67907,   0.926666,  -0.907707,  -0.00747479,
        -0.00815844, -0.975298,  -0.0227348, -0.842961, -0.0856783, 0.265912,
        0.0350528,   0.334472,   0.289027,   -0.173865, 0.989117,   0.577764,
        -0.583729,   -0.468592,  0.0303303,  -0.275404};
  ...
  for (int k = 0; k < n; k++) {
    for (int i = 0; i < n; i++) {
      for (int j = 0; j < n; j++) {
        printf("%f + i%f   ", xo[i + n * (j + n * k)].real(),
               xo[i + n * (j + n * k)].imag());
      }
      printf("\n");
    }
